...
export const syncStateToUrl = () => {
  ...
  if (state.timePeriod && state.timePeriod !== "month")
    params.set("time", state.timePeriod);
...
export const syncUrlToState = () => {
  ...
  state.timePeriod =
    params.get("time") || localStorage.getItem("defaultTimePeriod") || "month";
...
export const createUrl = (overrides = {}) => {
  ...
  const artifactName =
    overrides.artifactName !== undefined
  ...
  if (artifactName) {
    ...
    const artifactVersion =
      overrides.artifactVersion !== undefined
  ...
  const search =
    overrides.search !== undefined ? overrides.search : state.searchQuery;
  if (search) params.set("search", search);

  const hSearch =
    overrides.hSearch !== undefined
      ? overrides.hSearch
      : state.historySearchQuery;
  if (hSearch) params.set("hSearch", hSearch);

  const page =
    overrides.page !== undefined ? overrides.page : state.currentPage;
